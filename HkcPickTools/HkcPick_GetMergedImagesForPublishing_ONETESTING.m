...
%% Set adequate trimming patameters for figures 
...
im0=imread(filename);
[a,b,~]=size(im0);
imshow(im0(ceil(UP*a):floor(DW*a),ceil(LF*b):floor(RT*b),:))

figure(2)
filename=fullfile(one_dir,ls(fullfile(one_dir,'*_hk_after.png')));
im0=imread(filename);
[a,b,~]=size(im0);
imshow(im0(ceil(UPhk*a):floor(DWhk*a),ceil(LFhk*b):floor(RThk*b),:))
